...
export function DecisionPipeline() {
  ...
  return (
    <section id="intelligence" className="py-24 sm:py-32 border-t border-white/[0.06] scroll-mt-20">
      <div className="mx-auto max-w-7xl px-6 sm:px-10">
        ...
        <div className="border border-white/[0.08] rounded-xl overflow-hidden bg-[#111412]">
          {/* Top Stage Selectors */}
          <div className="grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-9 divide-x divide-y lg:divide-y-0 divide-white/[0.08]">
              ...
              return (
                <button
                  ...
                  className={`p-4 text-left transition-colors cursor-pointer relative group outline-none ${
                    isActive ? 'bg-[#151816]' : 'hover:bg-[#141715]'
                  }`}
                >
                  <div
                    className={`absolute top-0 inset-x-0 h-[2px] transition-colors ${
                      isActive ? 'bg-[#10B981]' : 'bg-transparent group-hover:bg-white/[0.15]'
                    }`}
                  />
                  <div className="text-[10px] font-mono text-[#7A807B] mb-1">
                    {stage.step}
                  </div>
                  <div className={`text-xs font-medium truncate ${
                    isActive ? 'text-[#F1F3EF]' : 'text-[#9A9F9B] group-hover:text-[#F1F3EF]'
                  }`}>
                    {stage.name}
                  </div>
                </button>
          ...
          <div className="p-6 sm:p-10 border-t border-white/[0.08] bg-[#0E110F]">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              <div className="lg:col-span-8 space-y-3">
                ...
                <p className="text-sm sm:text-base text-[#9A9F9B] leading-relaxed max-w-2xl">
                  {current.desc}
                </p>
              </div>

              <div className="lg:col-span-4 p-5 rounded-lg border border-white/[0.06] bg-[#141715]">
                <div className="text-[11px] font-mono text-[#7A807B] mb-1.5">
                  SYSTEM TELEMETRY
                </div>
                <div className="text-xs text-[#F1F3EF] leading-relaxed font-mono">
                  {current.note}
                </div>
              </div>
            </div>

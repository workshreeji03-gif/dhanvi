...
export function DecisionPipeline() {
  ...
  return (
    <section id="intelligence" className="py-24 sm:py-32 border-t border-[#E4E8E4] bg-white scroll-mt-20">
      <div className="mx-auto max-w-7xl px-6 sm:px-10">
        ...
        <div className="border border-[#E4E8E4] rounded-xl overflow-hidden bg-white shadow-xs">
          {/* Top Stage Selectors */}
          <div className="grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-9 divide-x divide-y lg:divide-y-0 divide-[#E4E8E4]">
                ...
                >
                  <div
                    className={`absolute top-0 inset-x-0 h-[2px] transition-colors ${
                      isActive ? 'bg-[#10B981]' : 'bg-transparent group-hover:bg-[#E4E8E4]'
                    }`}
                  />
                  <div className="text-[11px] font-mono tabular-nums text-[#8B928C] mb-1 font-medium">
                    {stage.step}
                  </div>
                  <div
                    className={`text-xs font-semibold tracking-[-0.015em] truncate ${
                      isActive ? 'text-[#111411]' : 'text-[#5F665F] group-hover:text-[#111411]'
                    }`}
                  >
                    {stage.name}
                  </div>
          ...
          <div className="p-6 sm:p-10 border-t border-[#E4E8E4] bg-[#F7F9F7]">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              <div className="lg:col-span-8 space-y-3">
                <div className="flex items-center gap-3">
                  <span className="text-xs font-mono tabular-nums text-[#10B981] font-semibold">
                    STAGE {current.step} / 09
                  </span>
                  <span className="text-[#D8DDD8]">•</span>
                  <span className="text-xs font-medium text-[#5F665F]">
                    {current.tagline}
                  </span>
                </div>
                <h3 className="text-2xl sm:text-3xl font-bold text-[#111411] tracking-[-0.03em]">
                  {current.name}
                </h3>
                <p className="text-sm sm:text-base text-[#5F665F] leading-relaxed max-w-2xl font-normal tracking-[-0.01em]">
                  {current.desc}
                </p>
              </div>

              <div className="lg:col-span-4 p-5 rounded-lg border border-[#E4E8E4] bg-white">
                <div className="text-[10px] font-mono font-semibold tracking-wider text-[#8B928C] mb-1.5 uppercase">
                  SYSTEM TELEMETRY
                </div>
                <div className="text-xs font-medium text-[#111411] leading-relaxed">
                  {current.note}
                </div>
              </div>
            </div>
